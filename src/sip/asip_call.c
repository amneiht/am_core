...
ACORE_LOCAL void _asip_call_module_init(pj_pool_t *pool) {
	...
	inv_list = acore_list_create(pool, NULL);
}
ACORE_LOCAL void _asip_call_module_close() {
	BEGIN
	acore_list_destroy(inv_list);
...
void* asip_call_add_media_support(const asip_call_media *module) {
	...
	// module clear
	call->clear = clear_module;
	call->user_data = call;
	//user clear
	call->mod.clear = module->clear;
	call->mod.user_data = module->user_data;
	// media
	call->counter = 0;
...
static void clear_module(void *mcall) {
	...
	if (call->mod.clear)
		call->mod.clear(call->mod.user_data);
...
// check using module
	...
	BEGIN
	call_module *call = data;
	if (call->counter == 0)
		clear_module(call);
	else {
		call->is_close = PJ_TRUE;
		res = PJ_FALSE;
	}
...
acore_check_t* asip_call_add_control(pj_pool_t *pool, const char *name,
	...
	res = acore_work_add_check(pool,
			acore_work_get_id(_asip_get_sys()->call_check), name, check, clear,
			user_data);
	END
	return res;
}
...
void asip_ua_call(asip_ua *ua, const pj_str_t *callee, const char *media_type,
	...
	BEGIN
	call_module *call_mod = acore_list_search(type_list, find_by_type, type);
	if (call_mod && !call_mod->is_close) {
		...
		if (status != PJ_SUCCESS) {
			...
			// tmp call
			acore_event_send(asip_event_id(), ASIP_EVENT_INV_STATE_START_FALSE,
					tmp);
			return;
		}
		...
		inv_bind *inb = acore_list_ele_create(inv->pool, sizeof(inv_bind));
		inb->call = scall;
		inb->inv = inv;
		scall->user_data = inb;
		acore_list_add(inv_list, inb);
		pj_list_insert_after(&ua->call, scall);
		scall->mod = call_mod;
		// add refrence
		call_mod->counter++;
		// modyfine invite session
		call_mod->on_start_call(scall, call_mod->mod.user_data);
		status = pjsip_inv_invite(inv, &tdata);
		status = pjsip_inv_send_msg(inv, tdata);
		scall->state = app_call_state_wait;

		acore_event_send(asip_event_id(), ASIP_EVENT_INV_STATE_START, scall);
	} else {
